...
namespace Riganti.Utils.Infrastructure.Services.Logging
{
    public class EmailLogger : LoggerBase
    {
        ...
        public EmailLogger(string recipientAddress, MailerService mailerService, IEnumerable<IAdditionalDataProvider> additionalDataProviders = null) : base(additionalDataProviders)
        {
        ...
        protected override void LogMessageCore(string message, IDictionary<string, string> additionalData, Severity severity)
        {
            ...
            output = $"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}<br />" + output; 
